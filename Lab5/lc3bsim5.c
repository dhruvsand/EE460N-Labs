...
int GetSetMDR1(int *x)           { return(x[SET_MDR_1]); }
int GetSetMDR0(int *x)           { return(x[SET_MDR_0]); }
int GetGotoRET(int *x)           { return(x[GotoRET]); }

int GetCOND(int *x)          { return((x[COND2] << 2) +(x[COND1] << 1) + x[COND0]); }
int GetRet(int *x)             { return((x[R5] << 5) + (x[R4] << 4) + (x[R3] << 3) + (x[R2] << 2) + (x[R1] << 1) + x[R0]); }
...
int GetLD_TEMPMDR(int *x)         { return(x[LD_TEMPMDR]); }
...
int GetMAR_SEL(int *x)         { return((x[MAR_SEL_1] << 1) + x[MAR_SEL_0]); }
int GetMDR_SEL(int *x)      { return(x[MDR_SEL]); }
...
int main(int argc, char *argv[]) {                              
    ...
    if(DEBUG) {
        ...
        argv[3] = "add.o";
        argv[4] = "data.o";
        argv[5] = "except_prot.o";
        argv[6] = "except_unaligned.o";
        argv[7] = "except_unknown.o";
        argv[8] = "int.o";
        argv[9] = "vector_table.o";
        argv[10]= "except_page.o";
        argc = 11;
    }
...
void eval_micro_sequencer() {
    ...
    if(IRD){
        ...
        if(condition==0){
            ...
            if(CURRENT_LATCHES.MEM_ALIGN)
                next_state = 39;
        } else if(condition==6){
            if(CURRENT_LATCHES.MEM_PROT&&CURRENT_LATCHES.MEM_PAGE)
                next_state=39;

        }
    }

    if(GetGotoRET(current_instruction))
        next_state = CURRENT_LATCHES.RetJ;
...
void cycle_memory() {
    ...
    if(GetMIO_EN(current_instruction)){
        ...
        if(current_mem_cycle==MEM_CYCLES){
        ...
        }else if(current_mem_cycle==(MEM_CYCLES-1)){
...
void latch_datapath_values() {
    ...
    if(GetLD_MAR(current_instruction)){
        if(GetMAR_SEL(current_instruction)==0)
            NEXT_LATCHES.MAR = BUS;
        else if(GetMAR_SEL(current_instruction)==1){
            //PTBR + VPN
            NEXT_LATCHES.MAR = (CURRENT_LATCHES.PTBR&0xFF00) + (((CURRENT_LATCHES.VA>>9)&0x007F)<<1);
        } else{
            //PFN + VA
            NEXT_LATCHES.MAR = (CURRENT_LATCHES.MDR&PTE_PFN_MASK) + (CURRENT_LATCHES.VA&0x01FF);
        }

    }
    if(GetLD_MDR(current_instruction)){

        if(GetMDR_SEL(current_instruction)==1){
            NEXT_LATCHES.MDR= CURRENT_LATCHES.TEMP_MDR;
        } else {
    ...
    if(GetSetMDR1(current_instruction)){
        NEXT_LATCHES.MDR= CURRENT_LATCHES.MDR|0x0002;
    }
    if(GetSetMDR0(current_instruction)){
        NEXT_LATCHES.MDR= CURRENT_LATCHES.MDR|0x0001;
    }

    if(((NEXT_LATCHES.MDR&0x0008)==0)&&(NEXT_LATCHES.PRIV==1 ))
        NEXT_LATCHES.MEM_PROT=1;
    else
        NEXT_LATCHES.MEM_PROT=0;

    if(NEXT_LATCHES.MDR&0x0004)
        NEXT_LATCHES.MEM_PAGE=0;
    else
        NEXT_LATCHES.MEM_PAGE=1;


    if(GetLD_RET(current_instruction))
        NEXT_LATCHES.RetJ=GetRet(current_instruction);

    if(GetLD_VA(current_instruction))
        NEXT_LATCHES.VA=BUS;

    if(GetLD_TEMPMDR(current_instruction))
        NEXT_LATCHES.TEMP_MDR=BUS;


    if(GetLD_EXCV(current_instruction)){
        if(CURRENT_LATCHES.MEM_PAGE&&(GetCOND(current_instruction)==6))
            NEXT_LATCHES.EXCV = 0x0002;
        if(CURRENT_LATCHES.MEM_PROT&&(GetCOND(current_instruction)==6))
            NEXT_LATCHES.EXCV = 0x0004;
        if(CURRENT_LATCHES.MEM_ALIGN&&(GetCOND(current_instruction)==5))
