    ...
    do {
        ...
        { printf("%s,     %s,    %s,    %s,    %s,    %s \n",lLabel,lOpcode,lArg1,lArg2,lArg3,lArg4);
            ...
            if(*lLabel!='\0'){
                ...
                symbolCount+=1;
                printf("The value: %d and the name:  %s \n", current_Address,newLable);
            }
            current_Address+=1;
        }
    } while( lRet != DONE );
    ...
    do {
        ...
        { printf("%s,     %s,    %s,    %s,    %s,    %s \n",lLabel,lOpcode,lArg1,lArg2,lArg3,lArg4);
            ...
            if(strcmp(isOpcode(lOpcode),"-1")==0){
                ...
                if(strcmp(lOpcode,".orig")==0){
                    current_Address = toNum(lArg1);
                    for( int i = 0; i < strlen( lArg1 ); i++ ) {
                        if(lArg1[i]=='x')
                            lArg1[i] = (char) tolower(lArg1[i]);
                        else
                            lArg1[i] = (char) toupper(lArg1[i]);
                    }
                    fprintf(outfile, "0");
                    fprintf(outfile, lArg1);
                    fprintf(outfile, "\n");
                ...
                if(strcmp(lOpcode,"add")==0||strcmp(lOpcode,"and")==0||strcmp(lOpcode,"xor")==0){
                    strcpy(&instruction[4],regToBin(lArg1));
                    strcpy(&instruction[7],regToBin(lArg2));
                    if(*lArg3=='r'){
                        strcpy(&instruction[10],"000");
                        strcpy(&instruction[13],regToBin(lArg3));
                    }
                    else if(*lArg3=='#'||*lArg3=='x'){
                        strcpy(&instruction[10],"1");
                        strcpy(&instruction[11],intToBin(toNum(lArg3),5));
                    }
                    else{
                        exit(4);
                    }
                    ...
                    for(int i=0; i<symbolCount;i++){
                        ...
                        if(strcmp(stable[i].name,lArg1)==0){
                            ...
                            strcpy(&instruction[7],intToBin(value,9));
                            break;
                        }
                    }
                }
                    ...
                    strcpy(&instruction[7],regToBin(lArg1));
                    ...
                    for(int i=0; i<symbolCount;i++){
                        ...
                        if(strcmp(stable[i].name,lArg1)==0){
                            int value = stable[i].value-current_Address;
                            strcpy(&instruction[5],intToBin(value,11));
                            break;
                        }
                    }
                }
                    ...
                    strcpy(&instruction[4],regToBin(lArg1));
                    strcpy(&instruction[7],regToBin(lArg2));
                    strcpy(&instruction[10],intToBin(toNum(lArg3),6));
                    ...
                    strcpy(&instruction[12],intToBin(toNum(lArg3),4));
                }
                else if(strcmp(lOpcode,"rshfl")==0){
                    strcpy(&instruction[4],regToBin(lArg1));
                    strcpy(&instruction[7],regToBin(lArg2));
                    strcpy(&instruction[10],"01");
                    strcpy(&instruction[12],intToBin(toNum(lArg3),4));
                }
                    ...
                    if(*lArg1!='x'&&*lArg1!='X')
                        exit(4);
                    strcpy(&instruction[8],intToBin(toNum(lArg1),8));
                }
                else if(strcmp(lOpcode,"halt")==0){
                    strcpy(&instruction[4],"0000");
                    strcpy(&instruction[8],intToBin(toNum("x25"),8));
                }
                ...
                fprintf(outfile,binToHex(instruction));
                fprintf(outfile, "\n");
            }
            current_Address+=1;
        }
...
char * intToBin(int num, int base){
    int temp=num;
    char* res= malloc(base+1);
    int bnum[base];
    if(num<0) {
        if(base==8)
            exit(3);
